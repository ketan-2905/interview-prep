...
export class AudioWorkletProcessor {
  ...
  async start(onAudioData: (data: Blob) => void) {
    ...
    // Buffer size 4096 balances latency & performance
    this.processor = this.audioContext.createScriptProcessor(4096, 1, 1);
    ...
    this.processor.onaudioprocess = (e) => {
      ...
      // SAFELY copy the exact bytes into a new ArrayBuffer (guaranteed ArrayBuffer type)
      const byteLength = pcmData.byteLength;
      const arrayBuffer = new ArrayBuffer(byteLength);
      const u8 = new Uint8Array(arrayBuffer);
      u8.set(new Uint8Array(pcmData.buffer, pcmData.byteOffset, byteLength));

      // Send Blob using this ArrayBuffer
      onAudioData(new Blob([arrayBuffer]));
    };
  }
